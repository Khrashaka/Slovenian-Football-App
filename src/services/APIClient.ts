...
class APIClient {
  ...
  async getNews(): Promise<NewsArticle[]> {
    ...
    try {
      const [generalNews, mins90News, onefootballNews, espnNews] = await Promise.allSettled([
        this.newsAPI.get('/news'),
        this.newsAPI.get('/news/90mins'),
        this.newsAPI.get('/news/onefootball'),
        this.newsAPI.get('/news/espn')
      ]);
      ...
      if (generalNews.status === 'fulfilled') {
        allNews.push(...(generalNews.value.data.articles || []));
      }
      if (mins90News.status === 'fulfilled') {
        allNews.push(...(mins90News.value.data.articles || []));
      }
      if (onefootballNews.status === 'fulfilled') {
        allNews.push(...(onefootballNews.value.data.articles || []));
      }
      if (espnNews.status === 'fulfilled') {
        allNews.push(...(espnNews.value.data.articles || []));
      }

      // Deduplicate by URL and filter for NK Maribor
      const uniqueNews = this.deduplicateNews(allNews);
      const mariborNews = this.filterMariborNews(uniqueNews);

      this.setCache(cacheKey, mariborNews, this.CACHE_TTL.NEWS);
      return mariborNews;
    } catch (error) {
